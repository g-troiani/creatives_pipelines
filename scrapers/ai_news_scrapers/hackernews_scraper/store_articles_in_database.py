#
# SCRAPERS/AI_NEWS_SCRAPERS/HACKERNEWS_SCRAPER//STORE_ARTICLES_IN_DATABASE.PY
# 
...
def store_articles_in_database(articles):
    ...
    for article in articles:
        ...
        cursor.execute("SELECT id FROM hacker_news WHERE article_id = ?", (article["URL"],))
        result = cursor.fetchone()

        if result is None:
            # Insert the article into the database
            cursor.execute("""
                INSERT INTO hacker_news (article_id, text, summary, url, page_url, title, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                article["URL"],
                article["Text"],
